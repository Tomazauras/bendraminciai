...
class Post {
    static async addPost(userData) {
        try {
            const { title,
                ...
                const postRef = await firebaseAdmin.firestore().collection("posts").doc(postId).set({
                    title,
                    description,
                    price,
                    state,
    ...
    static async addImageToDatabase(url){
        const posts_images_id = firebaseAdmin.firestore().collection("posts_images").doc().id;
        const postRef = await firebaseAdmin.firestore().collection("posts_images").doc(posts_images_id).set({
            url
        });
    ...
    static async getAllPosts() {
        try {
            const snapshot = await firebaseAdmin.firestore().collection("posts").get();
